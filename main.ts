...
namespace Microbit {
    ...
    */
    ...
    export function rockerori(orientation: rock): boolean {
        ...
        let ud = GetBuff2.getNumber(NumberFormat.Int8BE, 1)
        let lr = GetBuff2.getNumber(NumberFormat.Int8BE, 2)
        let flag
        if (orientation == 1) {
            if (ud > 50)
                flag = true
    ...
    export function RfidWrite(sector: RfidSector, block: RfidBlock, txt: string): void {
        ...
        for (let l = 0; l < len; l++) {
            ...
            buf3[3 + l] = txt.charCodeAt(l)

        }

        pins.i2cWriteBuffer(RFID_ADDR, buf3)
    ...
    export function RfidRead(sector: RfidSector, block: RfidBlock): string {
        ...
        pins.i2cWriteBuffer(RFID_ADDR, buf4)

        while (retry) {

            basic.pause(100);

            let stat2 = i2cread(RFID_ADDR, RFID_STATUS);

            if (stat2 == RfidStat.READ_SUCC) {
                ...
                for (let m = 0; m < 16; m++) {
                    ...
                    if (rxbuf[m] >= 0x20 && rxbuf[m] < 0x7f) {

                        ret += String.fromCharCode(rxbuf[m]) // valid ascii
    ...
    function RfidStop(): void {
        ...
        pins.i2cWriteBuffer(RFID_ADDR, buf5)
    ...
    export function Writemotorrelativetime(motoraddress: enMotorcolor, speed: number, time: number): void {
        ...
        while (flag4 != 7) {
            let GetBuff5 = pins.i2cReadBuffer(motoraddress, 6)
            flag4 = GetBuff5.getNumber(NumberFormat.Int8BE, 5);

        }
        while (flag4 != 11) {
            let GetBuff6 = pins.i2cReadBuffer(motoraddress, 6)
            flag4 = GetBuff6.getNumber(NumberFormat.Int8BE, 5);
    ...
    export function WriteDmotorlocation(speed1: number, speed2: number, location: number): void {
        ...
        if (speed2 < 0) {
            ...
            location_Buff23 = location_Buff23 | 0x8000
        ...
        let flag5 = GetBuff7.getNumber(NumberFormat.Int8BE, 5)//获取了第6位
        //接着读一次电机2
        ...
        while (true) {//如果没有在运行，则一直读取
            ...
            GetBuff7 = pins.i2cReadBuffer(caraddress2, 6)
            flag5 = GetBuff7.getNumber(NumberFormat.Int8BE, 5)
            GetBuff1 = pins.i2cReadBuffer(caraddress1, 6)
        ...
        while (true) {
            ...
            GetBuff1 = pins.i2cReadBuffer(caraddress1, 6)
            flag1 = GetBuff1.getNumber(NumberFormat.Int8BE, 5)
            if ((flag1 == 11) && (flag5 == 11)) break;//当有两个电机都执行完毕的时候，就退出死循环
    ...
    export function Readmotorlocation(motoraddress: enMotorcolor): number {
        ...
        GetBuff22 = pins.i2cReadBuffer(motoraddress, 5)

        let location_Buff

        let This_location

        location_Buff = (GetBuff22.getNumber(NumberFormat.Int8BE, 1) << 8) + GetBuff22.getNumber(NumberFormat.Int8BE, 2)
